...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
          bool canIncreaseWeight = IsWeighted && 
                (studentType == StudentType.Honors || studentType == StudentType.DualEnrolled);
            switch (letterGrade)
            {
                case 'A':
                    return canIncreaseWeight ? 5 : 4;
                case 'B':
                    return canIncreaseWeight ? 4 : 3;
                case 'C':
                    return canIncreaseWeight ? 3 : 2;
                case 'D':
                    return canIncreaseWeight ? 2 : 1;
                case 'F':
                    return canIncreaseWeight ? 1 : 0;
            }
            return 0;
        }
